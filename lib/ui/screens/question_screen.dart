...
class QuestionScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final widthForQuestion = test != null ? width/test.questions.length : width;
                            ...
                            Row(
                              ...
                              children: <Widget>[
                                ...
                                FlatButton(
                                  ...
                                  onPressed: () {
                                    Navigator.of(context).pop();
                                    BlocProvider.of<QuestionCubit>(context).finishTest(state.test.questions[state.currentQuestionInd].options[state.currentChoice]);
                                },),
                              ],
                            )
                          ],
                            ...
                            return MapEntry(optionIndex, Container(
                              ...
                              child: RadioListTile(
                                ...
                                title: Text(
                                  ...
                                  style: TextStyle(
                                    fontSize: 18,
                                    fontWeight: (state.currentChoice == optionIndex) ? FontWeight.bold : FontWeight.normal,
                                  ),
                                ),
                                value: optionIndex,
                                groupValue: state.currentChoice,
                                activeColor: Theme.of(context).colorScheme.secondary,
                                onChanged: (value) {
                                    print(state.currentChoice);
                                    BlocProvider.of<QuestionCubit>(context).showSelectedChoice(value);
                                },
                              ),
                            )
                            );
                        ...
                        onPressed: () {
                          if (state.test.questions.length > state.currentQuestionInd+1) {
                            BlocProvider.of<QuestionCubit>(context)
                                .getNextQuestion(state.test.questions[state
                                .currentQuestionInd].options[state
                                .currentChoice]);
                          } else {
                            BlocProvider.of<QuestionCubit>(context)
                                .finishTest(state.test.questions[state
                                .currentQuestionInd].options[state
                                .currentChoice]);
                          }
                        },
